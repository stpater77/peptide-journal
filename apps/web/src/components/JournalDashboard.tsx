  ...
  return (
    <main className="min-h-screen bg-[#f6f5ed] text-[#17211c]">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 border-b border-[#dfd8c6] pb-5 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm font-medium text-[#c65f18]">
          ...
          <nav className="flex gap-4 text-sm font-medium">
            <Link
              href="/privacy"
              className="text-[#0f766e] underline-offset-4 hover:text-[#c65f18] hover:underline"
            >
              Privacy
            </Link>
            <Link
        ...
        <section className="grid gap-3 sm:grid-cols-3">
          {DASHBOARD_STATS.map((stat) => (
            <div
              key={stat.label}
              className={`rounded-lg border border-[#dfd8c6] border-l-4 bg-white px-4 py-3 shadow-sm ${stat.accentClass}`}
            >
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
          ...
          <aside>
            <section className="rounded-lg border border-[#dfd8c6] bg-white p-5 shadow-sm">
              <p className="text-sm font-medium text-[#c65f18]">
                Protocol Overview
              </p>
              ...
              <div className="mt-4 space-y-3">
                {PROTOCOL_GROUPS.map((group) => (
                  <div
                    key={group.title}
                    className="rounded-lg border border-[#e7dcc9] bg-[#fffaf1] p-3"
                  >
